...
int main(int argc, char* argv[])
{
	...
	@autoreleasepool {
		...
		// Clean up respring artifacts so banking apps (e.g. Revolut) cannot detect
		// the userspace reboot via crash/jetsam log files left by SpringBoard.
		// We also schedule a delayed cleanup to catch logs written asynchronously.
		void (^cleanRespringLogs)(void) = ^{
			NSFileManager *fm = [NSFileManager defaultManager];
			NSString *crashDir = @"/var/mobile/Library/Logs/CrashReporter";
			NSArray<NSString *> *files = [fm contentsOfDirectoryAtPath:crashDir error:nil];
			for (NSString *f in files) {
				if ([f hasPrefix:@"SpringBoard_"] ||
					[f hasPrefix:@"JetsamEvent_"] ||
					[f hasPrefix:@"UIKitApplication:com.apple.springboard"]) {
					[fm removeItemAtPath:[crashDir stringByAppendingPathComponent:f] error:nil];
				}
			}
		};
		cleanRespringLogs();
		dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(8 * NSEC_PER_SEC)), dispatch_get_main_queue(), cleanRespringLogs);
